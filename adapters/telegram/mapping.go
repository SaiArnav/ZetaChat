...
func (a *Adapter) Messages(chatID string) ([]core.Message, error) {
	...
	if !ok {
		return nil, fmt.Errorf("telegram: unexpected history response %T", hist)
	}

	a.buildIndex(mod.GetUsers(), mod.GetChats())

	// History is newest-first; flip to oldest-first.
	out := make([]core.Message, 0, len(mod.GetMessages()))
...
func (a *Adapter) Search(query string) ([]core.Message, error) {
	...
	if !ok {
		return nil, fmt.Errorf("telegram: unexpected search response %T", res)
	}

	a.buildIndex(mod.GetUsers(), mod.GetChats())
	a.refreshPeers(mod.GetUsers(), mod.GetChats())
